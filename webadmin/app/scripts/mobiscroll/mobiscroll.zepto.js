...
(function ($, window, document, undefined) {
  ...
  classes.Scroller = function (el, settings, inherit) {
     ...
     * @param {Object} obj { idx: n, data: { keys: [], values: [], ... } }.
     * @param {Number} [time=0] Animation time when scrolling to the selected value on the new wheel.
     * @param {Boolean} [manual=false] Indicates that the change was triggered by the user or from code.
     */
    that.changeWheel = function (obj, time, manual) {
      if ($markup) {
        wheels[obj.idx] = obj.data;
        $('.dwwl' + obj.idx + ' .dw-ul', $markup).html(generateWheelItems(obj.idx));
        that.position();
...
(function ($, undefined) {
  ...
  ms.datetime = {
     ...
     */
    formatDate: function (format, date, settings, value) {
      if (!date) {
        return null;
      }
      ...
      if (settings.preset === 'dateinterval') {
        output += value[3] ? (' ' + value[3]) : '';
      }
      return output;
    },
    /**
...
(function ($, undefined) {
    ...
    preset = function (inst) {
      ...
      if (p.match(/dateinterval/i)) {
        ...
        for (var i = 0; i < 96; i += 2) {
          keys.push(result[i]);
          values.push(result[i] + ' - ' + result[i + 1]);
        }
        wheels.push([{
          keys: values,
          values: values,
      ...
      return {
        ...
        formatValue: function (d) {
          return datetime.formatDate(format, getDate(d), s, d);
        },
        parseValue: function (val, inis) {
          if (!val) {
            innerValues = {};
          }
          var temp = getArray(val ? datetime.parseDate(format, val, s) : (s.defaultValue || new Date()), !!val && !!val.getTime);
          if (inis.settings.preset === 'dateinterval') {
            temp.push(val.substring(val.indexOf(' ') + 1));
          }
          return temp;
        },
        validate: function (dw, i, time, dir, inis) {
          ...
          if (inis.settings.preset === 'dateinterval') {
            temp.push(inis._tempWheelArray[3]);
          }
  ...
  $.each(['date', 'time', 'datetime', 'dateinterval'], function (i, v) {
