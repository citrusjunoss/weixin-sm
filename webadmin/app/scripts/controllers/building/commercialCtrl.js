...
(function () {
  ...
  angular.module('webApp').controller('commercialCtrl', commercialCtrl);
  function commercialCtrl($scope, commonServ, HOST, $location) {
    $scope.params = {
      house_name: '',   //楼盘名称
      province: '',     //省名
      province_id: '',  //省ID
    ...
    function getDetails() {
      ...
      commonServ.post({
        ...
        success:function(data){
          $scope.commercialList = data.errmsg.data;
          $scope.isLoading = {
            display: "none"
          };
        },
      ...
      commonServ.post({
        ...
        success:function(data){
          if (data.errmsg.data.length == 0) {
            ...
            $scope.commercialList = $scope.commercialList.concat(data.errmsg.data);
            int++;
            isajax = false;
          }
        },
  ...
  commercialCtrl.$inject = ['$scope', 'commonServ', 'HOST', '$location'];
})();
